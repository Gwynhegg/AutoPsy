...
namespace AutoPsy.Logic
{
    ...
    public static class PoolModelling
    {
        ...
        {
            var rightHandStepping = new List<float>();      // массив, содержащий элементы правостороннего обхода
            var leftHandStepping = new List<float>();       // массив, содержащий элементы левостороннего обхода

            var leftCurrentState = baseState[0];

            for (var i = 0; i < baseState.Count; i++)       // последовательно идем по значениям слева направо
            {
            ...
            var rightCurrentState = baseState[baseState.Count - 1];
            for (var i = baseState.Count - 1; i >= 0; i--)      // последовательно идет по значениям справа налево
            {
            ...
            List<float> result = GetAverageValues(baseState, leftHandStepping, rightHandStepping);      // получаем результирующее значение вычислением среднего
        ...
        {
            var result = new List<float>();     // инициализируем результирующий массив
